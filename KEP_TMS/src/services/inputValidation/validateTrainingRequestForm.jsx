...
export const validateTrainingRequestForm = (formData)=>{
  ...
  //  if(!formData.requestorBadge) throw new Error("Requestor ID is required");
    ...
    if(!formData.trainingFacilitators) throw new Error("Training facilitator(s) is/are required");
    if(!formData.trainingObjectives) throw new Error("Training objectives is required");
    if(!formData.venue) throw new Error("Venue is required");
    if(!formData.trainingProvider?.id) throw new Error("Training Provider is required");
