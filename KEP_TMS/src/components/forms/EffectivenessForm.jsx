...
import { performanceCharacteristicsArray, projectPerformanceEvaluationArray } from "../../services/constants/effectivenessConstant";
import { SessionGetEmployeeId } from "../../services/sessions";
const EffectivenessForm = ({ data, userData }) => {
  const [isAfter, setIsAfter] = useState(false);
  const [errors, setErrors] = useState({});
  const [performanceCharacteristics, setPerformanceCharacteristics] = useState(performanceCharacteristicsArray);
  const [projectPerformanceEvaluation, setProjectPerformanceEvaluation] =useState(projectPerformanceEvaluationArray);
const numItems = [0,1,2];
  ...
  const validateForm = ()=>{
    ...
    if (!getFormData.performanceCharacteristics.length) {
      ...
      numItems.forEach((i) => {
        ...
        if (!performanceCharacteristics[i]?.content) {
          formErrors.performanceCharacteristics = `Performance characteristic ${i+1} is required`;
          validForm = false;
        }
        if (performanceCharacteristics[i]?.rating === 0 ) {
          formErrors.performanceCharacteristics = `Rating for Performance characteristic ${i+1} is required`;
          validForm = false;
        }
      });
    }
    if (!getFormData.projectPerformanceEvaluation.length) {
      ...
      numItems.forEach((i) => {
        if (!projectPerformanceEvaluation[i].content) {
          formErrors.projectPerformanceEvaluation = `Project Performance Evaluation ${i+1} content is required`;
          validForm = false;
        }
        if (projectPerformanceEvaluation[i].performanceBeforeTraining === 0 ) {
          formErrors.projectPerformanceEvaluation = `Performance before training for Project Performance Evaluation ${i+1} is required`;
          validForm = false;
        }
        if (projectPerformanceEvaluation[i].projectedPerformance === 0) {
          formErrors.projectPerformanceEvaluation = `Projected performance for Project Performance Evaluation ${i+1} is required`;
          validForm = false;
        }
        if(isAfter){
        if (projectPerformanceEvaluation[i].actualPerformance === 0) {
          formErrors.projectPerformanceEvaluation = `Actual performance for Project Performance Evaluation ${i+1} is required`;
          validForm = false;
        }
        if (projectPerformanceEvaluation[i].evaluatedActualPerformance === 0) {
          formErrors.projectPerformanceEvaluation = `Evaluated actual performance for Project Performance Evaluation ${i+1} is required`;
          validForm = false;
        }}
    })}
      ...
      <Card.Body className="border-top ">
        ...
        <Form>
          ...
          <Form.Group>
            ...
            {errors?.performanceCharacteristics && <ErrorTemplate message={errors?.performanceCharacteristics}/>}
          </Form.Group>
          <br />
          <Form.Group>
            <b>
            ...
            <Row>
              <Col className={`d-flex gap-2 align-items-end`}>
                ...
                <span className="flex-grow-1 border-0 border-bottom">{getAfterTrainingDate().toString()}</span>
              </Col>
              <Col className={`d-flex gap-2 align-items-end`}>
                <label className="fw-bold" style={{ fontSize: "0.8rem" }}>
                  Evaluator
            ...
            <Table className="mt-2 table-bordered m-0">
              ...
              <tbody>
                ...
                <tr key={`evaluation${item}`}>
                  <th scope="row">{item+1}</th>
                  <td>
                    <textarea className="no-focus w-100 border-0" name="content" value={projectPerformanceEvaluation[item]?.content?? ""}
                  ...
                  <td style={{ verticalAlign: "middle" }}>
                    <Rating
                      className="justify-content-center"
                      value={projectPerformanceEvaluation[item]?.performanceBeforeTraining}
                  ...
                  <td style={{ verticalAlign: "middle" }}>
                    <Rating
                      className="justify-content-center"
                      value={projectPerformanceEvaluation[item]?.projectedPerformance}
                  ...
                  <td style={{ verticalAlign: "middle" }}>
                    <Rating
                      className="justify-content-center"
                      value={projectPerformanceEvaluation[item]?.actualPerformance}
                  ...
                  <td style={{ verticalAlign: "middle" }}>
                    <Rating
                      className="justify-content-center"
                      value={projectPerformanceEvaluation[item]?.evaluatedActualPerformance}
            ...
            {errors?.projectPerformanceEvaluation && <ErrorTemplate message={errors?.projectPerformanceEvaluation}/>}
          </Form.Group>
          <br />
          <Form.Group>
            <b>III. Comments / Remarks</b>
            <i> &#x28;to be filled up after the training&#x29; :</i>
            <textarea className="form-control" rows={3} placeholder="Comments/Remarks" disabled={!isAfter}></textarea>
          </Form.Group>
          <div className="text-end mt-3">
          <Button type="button" icon="pi pi-eraser" label="Reset" className="rounded" severity="secondary" onClick={()=>{
            setPerformanceCharacteristics(performanceCharacteristicsArray);
            setProjectPerformanceEvaluation(projectPerformanceEvaluationArray);
          }}/>
          <Button type="button" icon="pi pi-cloud-upload" label="Submit Form" className="rounded ms-2" severity="success" 
