...
const VideoAccessMatrixForm = ({ handleShow, handleClose, selectedData, onFinish }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if(!formData?.employeeBadge){
      setErrors(prev=>({...prev, approver: "No user selected" }));
      return;
    }
    confirmAction({
      showLoaderOnConfirm: true,
      title: selectedData ? "Update Video Access Matrix" : "Add Video Access Matrix",
      text: "Are you sure you want to submit?",
      onConfirm: () => 
        handleResponseAsync(
          () =>
            selectedData?.id
              ? videoAccessMatrixService.updateVideoAccessMatrix({
                ...formData,
                updatedBy: SessionGetEmployeeId(),
              })
              : videoAccessMatrixService.createVideoAccessMatrix({
                ...formData,
                createdBy: SessionGetEmployeeId(),
              }),
          (e) => {
            actionSuccessful("Success", e?.message ?? `Cost Approval Matrix successfully ${selectedData ? "updated" : "saved"}`);
            handleClose();
            onFinish();
          }
        )
